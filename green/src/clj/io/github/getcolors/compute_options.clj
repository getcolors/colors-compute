...
(defn apply-options [provider stage request documents]
 (let [selected (select-keys request [:backups :ipv6])]
  (if (empty? selected) documents
   (let [descriptor (get (json/parse-string (slurp (io/resource "colors_compute/compute-options.json")) true) (keyword provider)) backup (:backups selected)]
    ...
    (when (contains? selected :backups)
     (when-not (and (map? backup) (boolean? (:enabled backup)) (= (set (keys backup)) (if (:enabled backup) #{:enabled :schedule} #{:enabled}))) (fail "invalid compute backup policy"))
     (when (:enabled backup)
     ...
     (let [paths (keep (fn [[filename doc]]
      ...
      (update-in documents (first paths) (fn [resource]
       (cond-> resource
        ...
        (:enabled backup) (assoc (keyword (:schedule_field descriptor)) [{:type "daily" :hour (long (get-in backup [:schedule :hour]))}])))))))))
)
