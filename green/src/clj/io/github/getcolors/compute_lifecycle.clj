...
(defn- document? [doc]
  (and (exact? doc #{:schema_version :identity :revision :write_id :lock :generation :status :topology_declared :key :shared :nodes})
       ...
       (or (not= "retired" (:status doc)) (and (contains? #{"absent" "removed"} (get-in doc [:key :phase])) (all-destroyed? doc) (not-any? :desired (vals (:nodes doc)))))
  ...
  (and (map? event) (string? (:type event)) (str/starts-with? (:type event) "lifecycle/")
       (let [suffix (subs (:type event) 10) additional (get extras suffix)]
         (and additional (exact? event (into #{:type :run_id :write_id :target_etag} additional))
              ...
              (or (not (contains? event :evidence)) (= "readable-state" (:evidence event)) (and (contains? #{"shared-retry" "retry"} suffix) (= "verified-provider-absence" (:evidence event))))))))
(defn- transition [doc event suffix]
  (let [node-key (when (:node_id event) (keyword (:node_id event))) node (get-in doc [:nodes node-key])
    ...
    (case suffix
      ...
      "retire" (do (require-transition (and (= "deleting" (:status doc)) (contains? #{"absent" "removed"} (get-in doc [:key :phase])) (all-destroyed? doc))) (assoc doc :status "retired"))
