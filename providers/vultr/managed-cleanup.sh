...
[[ -n ${COLORS_PAR_VULTR_API_KEY:-} ]] || { log "compute credential missing"; exit 1; }
...
vultr_api() { printf '%s\n' "Authorization: Bearer ${COLORS_PAR_VULTR_API_KEY:-}" | curl --connect-timeout 10 --max-time 30 -fsS -H @- "https://api.vultr.com/v2$1"; }
if [[ -n ${COLORS_PAR_VULTR_API_KEY:-} ]]; then
  if [[ -n ${volume_ids:-} ]]; then
    verdict="unverified"
    for _ in $(seq 1 30); do
      if live=$(vultr_api "/blocks?per_page=500" 2>/dev/null | jq -er 'if (.blocks | type) == "array" and ((.links.pages.next // .meta.links.next // "") == "") then [.blocks[].id] | join("\n") else error("incomplete listing") end' 2>/dev/null); then
    ...
    case $verdict in
      ...
      *) log "FATAL: could not verify block-volume deletion against the Vultr API — check manually"; exit 1 ;;
    esac
  fi
  if [[ -n ${lb_ip:-} ]]; then
    verdict="unverified"
    ...
    case $verdict in
      ...
      *) log "FATAL: could not verify load-balancer deletion against the Vultr API — check manually"; exit 1 ;;
    esac
  fi
fi
