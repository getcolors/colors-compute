"""Check canonical hcloud renders and optionally validate provider schema in temporary dirs."""
import argparse
import copy
import json
import os
...
def check():
    ...
    assert 'apply_to' not in shared['resource']['hcloud_firewall']['network']
    node = json.loads((ROOT / 'examples/node/node.tf.json').read_text())
    assert node['resource']['hcloud_server']['node']['firewall_ids'] == inputs['firewall_ids']
    assert node['output']['params']['value']['vpc_ip'] == '${hcloud_server_network.node.ip}'
    assert render('{{value}}', {'value': True}) is True
    assert render('{{value}}', {'value': 24}) == 24
    assert render('{{value}}', {'value': ['a']}) == ['a']
    assert render('${hcloud_server.node.id}', {}) == '${hcloud_server.node.id}'
...
def main():
    ...
    if args.tofu:
        ...
        for mode in ('shared-keygen', 'shared-optout', 'node'):
            with tempfile.TemporaryDirectory(prefix='colors-compute-hcloud-') as directory:
