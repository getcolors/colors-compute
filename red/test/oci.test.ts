...
const opts={'provider-backend':'oci','provider-compute':'oci','oci-bucket':'demo-states','oci-region':'eu-frankfurt-1','oci-namespace':'namespace1','oci-compartment-id':'ocid1.compartment.example',profile:'demo','oci-bucket-mode':'managed','compute-prevent-destroy':false};
...
test('OCI recovery refuses provider resources including default boot-volume names',async()=>{
 ...
 for(const resourceName of [null,'Boot volume of instance demo-0']){
  const transitions:any[]=[];let released=false;
  ...
  const runner:BackendRunner=async(args)=>({exit:0,out:JSON.stringify(args[1]==='raw-request'?{status:'200 OK',data:{version:4,serial:1,lineage:'line',outputs:{},resources:[]},headers:{}}:{data:args[1]==='bv'&&resourceName?[{'display-name':resourceName,'lifecycle-state':'AVAILABLE'}]:[]}),err:''});
  const run=()=>recover_absent_oci_nodes({...opts,'oci-availability-domain':'ad1'},{'0':'attempt'},{},runner,()=>owner);
