...
export function applyOptions(provider:string,stage:string,request:Map,documents:Map):Map {
 ...
 if(keys.includes('ipv6'))resource[descriptor.ipv6_field]=request.ipv6;if(backup!==undefined){resource[descriptor.backups_field]=descriptor.backups_values[String(backup.enabled)];if(backup.enabled)resource[descriptor.schedule_field]=[{type:'daily',hour:backup.schedule.hour}];}return result;
}
