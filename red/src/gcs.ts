...
export async function gcsClient(environment:Map=process.env,runner:BackendRunner=executeBackendCommand){
 ...
 return async(method:string,path:string,body?:unknown,query:Map={})=>{
  const url=new URL('https://storage.googleapis.com/'+path);for(const [k,v] of Object.entries(query))url.searchParams.set(k,String(v));
