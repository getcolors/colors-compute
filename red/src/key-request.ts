...
export function key_request(opts:Map,prepared:Map,environment:Record<string,string|undefined>|null=null) {
 ...
 const kind=(recipes as Map)[provider].external_key_kind;
 ...
 if(opts['red/event']==='build'||opts['red/dry-run']===true)return {mode:'external',public_key:'ssh-ed25519 PLACEHOLDER managed-by-colors'};
 let value:string;
 if(kind==='content')value=publicKey(refs[0]);
 else if(kind==='public_file') {
