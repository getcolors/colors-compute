...
def apply_options(provider, stage, request, documents):
    ...
    if 'backups' in selected:
        if not isinstance(backup, dict) or type(backup.get('enabled')) is not bool or set(backup) != ({'enabled', 'schedule'} if backup['enabled'] else {'enabled'}):
