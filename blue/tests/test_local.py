...
from colors_compute.contract import validate, credential_requirements
...
def identity(opts):
    return {'profile': 'demo', 'provider': 'aws', 'backend': {'kind': 'local', 'path': opts['local-state-dir']}}
...
@pytest.mark.asyncio
async def test_converge_keeps_local_state_after_workdir_cleanup(tmp_path):
