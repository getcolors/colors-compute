...
Native bucket metadata updates use `POST /n/{namespace}/b/{bucket}` with JSON content type and the bucket ETag. `PUT` at that route returned404 during live finalization; it had already persisted the deleting owner marker. Retrying the normal finalizer with the corrected method resumes from that marker without rerunning application or compute deletion.
